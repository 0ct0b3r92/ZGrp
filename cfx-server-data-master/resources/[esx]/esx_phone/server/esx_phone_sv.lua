...
MySQL:open("localhost", "essentialmode", "user", "password")
		...
		if foundNumber then
			TriggerEvent('esx:getPlayerFromId', _source, function(xPlayer)
				...
				if phoneNumber == xPlayer.phoneNumber then
					TriggerClientEvent('esx:showNotification', _source, 'You can not add yourself')
				else
					...
					if hasAlreadyAdded then
						TriggerClientEvent('esx:showNotification', _source, 'This number is already in your contact list')
			...
			TriggerClientEvent('esx:showNotification', _source, 'This number is not assigned...')
		end
	end)
end)
-- ###########################################################################################################
				...
				for k, v in pairs(xPlayers) do
					...
					if phoneNumber == "police" then
						...
						if v.job.name == 'cop' then
							...
							job = "ALERTE POLICE"
							TriggerClientEvent('esx_phone:onMessage', v.player.source, xPlayer.phoneNumber, message, xPlayer.player.coords, {reply = 'Reply'}, anon, job)
						end
